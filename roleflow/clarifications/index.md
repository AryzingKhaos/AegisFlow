# clarifications/ 目录索引

## 目录说明

- 本目录存放由 Clarifier 输出的结构化需求文档
- 文档路径使用 `@roleflow/clarifications/[版本号]/[功能模块]-prd.md`
- 新增需求文档时，必须同步更新本索引和对应版本目录索引

## 版本列表

| 版本 | 索引 | 说明 |
|------|------|------|
| `0.1.0` | [0.1.0/index.md](0.1.0/index.md) | 当前版本需求文档目录 |

## 文档清单

| 版本 | 模块 | 文档 |
|------|------|------|
| `0.1.0` | `default-workflow-intake-layer` | [0.1.0/default-workflow-intake-layer-prd.md](0.1.0/default-workflow-intake-layer-prd.md) |
| `0.1.0` | `default-workflow-intake-error-explainability` | [0.1.0/default-workflow-intake-error-explainability-prd.md](0.1.0/default-workflow-intake-error-explainability-prd.md) |
| `0.1.0` | `default-workflow-intake-ink-ui` | [0.1.0/default-workflow-intake-ink-ui-prd.md](0.1.0/default-workflow-intake-ink-ui-prd.md) |
| `0.1.0` | `default-workflow-intake-project-workflows` | [0.1.0/default-workflow-intake-project-workflows-prd.md](0.1.0/default-workflow-intake-project-workflows-prd.md) |
| `0.1.0` | `default-workflow-task-debug-transcript` | [0.1.0/default-workflow-task-debug-transcript-prd.md](0.1.0/default-workflow-task-debug-transcript-prd.md) |
| `0.1.0` | `default-workflow-clarify-dialogue-artifact-reinjection` | [0.1.0/default-workflow-clarify-dialogue-artifact-reinjection-prd.md](0.1.0/default-workflow-clarify-dialogue-artifact-reinjection-prd.md) |
| `0.1.0` | `default-workflow-workflow-layer` | [0.1.0/default-workflow-workflow-layer-prd.md](0.1.0/default-workflow-workflow-layer-prd.md) |
| `0.1.0` | `default-workflow-role-layer` | [0.1.0/default-workflow-role-layer-prd.md](0.1.0/default-workflow-role-layer-prd.md) |
| `0.1.0` | `default-workflow-cli-streaming-output` | [0.1.0/default-workflow-cli-streaming-output-prd.md](0.1.0/default-workflow-cli-streaming-output-prd.md) |
| `0.1.0` | `default-workflow-role-codex-agent` | [0.1.0/default-workflow-role-codex-agent-prd.md](0.1.0/default-workflow-role-codex-agent-prd.md) |
| `0.1.0` | `default-workflow-role-codex-cli` | [0.1.0/default-workflow-role-codex-cli-prd.md](0.1.0/default-workflow-role-codex-cli-prd.md) |
| `0.1.0` | `default-workflow-role-codex-cli-output-passthrough` | [0.1.0/default-workflow-role-codex-cli-output-passthrough-prd.md](0.1.0/default-workflow-role-codex-cli-output-passthrough-prd.md) |
| `0.1.0` | `default-workflow-role-node-pty-subcommand` | [0.1.0/default-workflow-role-node-pty-subcommand-prd.md](0.1.0/default-workflow-role-node-pty-subcommand-prd.md) |
| `0.1.0` | `default-workflow-role-child-process-subcommand` | [0.1.0/default-workflow-role-child-process-subcommand-prd.md](0.1.0/default-workflow-role-child-process-subcommand-prd.md) |
| `0.1.0` | `default-workflow-role-prompt-bootstrap` | [0.1.0/default-workflow-role-prompt-bootstrap-prd.md](0.1.0/default-workflow-role-prompt-bootstrap-prd.md) |
| `0.1.0` | `default-workflow-final-artifact-markdown-output` | [0.1.0/default-workflow-final-artifact-markdown-output-prd.md](0.1.0/default-workflow-final-artifact-markdown-output-prd.md) |
| `0.1.0` | `default-workflow-phase-artifact-input-phases` | [0.1.0/default-workflow-phase-artifact-input-phases-prd.md](0.1.0/default-workflow-phase-artifact-input-phases-prd.md) |
| `0.1.0` | `default-workflow-intake-ui-theme-refinement` | [0.1.0/default-workflow-intake-ui-theme-refinement-prd.md](0.1.0/default-workflow-intake-ui-theme-refinement-prd.md) |
| `0.1.0` | `default-workflow-intake-output-structure-separation` | [0.1.0/default-workflow-intake-output-structure-separation-prd.md](0.1.0/default-workflow-intake-output-structure-separation-prd.md) |
| `0.1.0` | `default-workflow-intake-codex-style-output-stream` | [0.1.0/default-workflow-intake-codex-style-output-stream-prd.md](0.1.0/default-workflow-intake-codex-style-output-stream-prd.md) |
| `0.1.0` | `default-workflow-intake-codex-exec-failure-main-screen` | [0.1.0/default-workflow-intake-codex-exec-failure-main-screen-prd.md](0.1.0/default-workflow-intake-codex-exec-failure-main-screen-prd.md) |
| `0.1.0` | `default-workflow-task-debug-transcript-codex-io-only` | [0.1.0/default-workflow-task-debug-transcript-codex-io-only-prd.md](0.1.0/default-workflow-task-debug-transcript-codex-io-only-prd.md) |
| `0.1.0` | `default-workflow-intake-initial-requirement-bootstrap` | [0.1.0/default-workflow-intake-initial-requirement-bootstrap-prd.md](0.1.0/default-workflow-intake-initial-requirement-bootstrap-prd.md) |

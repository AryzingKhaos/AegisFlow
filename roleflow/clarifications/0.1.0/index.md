...
| `default-workflow-intake-codex-exec-failure-main-screen` | [default-workflow-intake-codex-exec-failure-main-screen-prd.md](default-workflow-intake-codex-exec-failure-main-screen-prd.md) | `default-workflow` 的 Intake UI 在 codex exec 中断时必须将结构化失败信息页提升为主屏幕主内容，而不是只显示任务失败提示的需求文档 |
...
| `default-workflow-intake-initial-requirement-bootstrap` | [default-workflow-intake-initial-requirement-bootstrap-prd.md](default-workflow-intake-initial-requirement-bootstrap-prd.md) | `default-workflow` 的 Intake 启动前置流程调整需求文档，要求先收集需求标题、基于标题推荐 workflow、提前创建 task 目录，并在正式执行前把详细描述或 PRD 路径写入 `tasks/<taskId>/artifacts/clarify/initial-requirement.md` |

...
- 显式固定 `critic` 的同名命名规则：角色原型映射 `/Users/aaron/code/roleflow/roles/critic.md`，AegisFlow 项目侧默认文件为 `.aegisflow/roles/critic.md`，override 仅作为未来可选扩展。
...
- `critic` 按“同名命名收敛”处理：原型层文件名与项目侧物化文件统一为 `critic.md`；本次只固定默认读取规则，不扩展新的别名策略。
...
- 当前 `src/default-workflow/role/prompts.ts` 已固定角色原型目录为 `/Users/aaron/code/roleflow/roles`；在原型文件更名后，`critic` 的默认文件解析也需要随之收敛到 `critic.md`，否则运行时装载链路会漂移。
...
- [x] 统一 `critic` 的原型层与项目侧文件命名为 `critic.md`，并确认项目侧默认读取仍为 `critic.md` 同名文件。
